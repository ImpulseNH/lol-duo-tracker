...
def get_account_data(gameName: str, tagLine: str, region: str = settings.DEFAULT_REGION) -> Optional[Dict[str, Any]]:
    """
    ...
    :return: The account's data or None if an error occurs.
    """
    api_key = get_api_key()
...
def get_matches_ids(puuid: str, start: int = 0, count: int = settings.DEFAULT_MATCH_COUNT, endTime: int = None, region: str = settings.DEFAULT_REGION) -> Optional[List[str]]:
    """
    ...
    :return: The summoner's match history or None if an error occurs.
    """
    api_key = get_api_key()
    ...
    if not puuid:
        print("Error: PUUID not provided.")
        return None
...
def get_match_data(match_id: str, region: str = settings.DEFAULT_REGION) -> Optional[Dict[str, Any]]:
    """
    Fetches the match data based on the match ID.
    
    :param match_id: The ID of the match.
    :param region: The region to query (default is settings.DEFAULT_REGION).
    :return: The match data or None if an error occurs.
    """
    api_key = get_api_key()
...
def get_last_match_timestamp(summoner1_puuid: str, summoner2_puuid: str, region: str = settings.DEFAULT_REGION) -> Optional[Dict[str, Any]]:
    """
    ...
    :return: Timestamp of the oldest last match (in seconds) or None if an error occurs
    """
    ...
    if not summoner1_last_match_data or not summoner2_last_match_data:
        return None
